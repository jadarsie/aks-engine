...
func (ccc *CustomCloudConfig) SetEnvironment() error {
	...
	// Add to python cert store the self-signed root CA generated by Azure Stack's CI as azure-cli complains otherwise
	azsSelfSignedCaPath := "/aks-engine/Certificates.pem"
	if _, err = os.Stat(azsSelfSignedCaPath); err == nil {
		cmd := exec.Command("/bin/bash", "-xc", fmt.Sprintf(`VER=$(python3 -V | grep -o [0-9].[0-9]*. | grep -o [0-9].[0-9]*);
		CA=/usr/local/lib/python${VER}/dist-packages/certifi/cacert.pem;
		cat ${CA}; if [ -f ${CA} ]; then cat %s >> ${CA}; else echo NOPE!; fi; cat ${CA}`, azsSelfSignedCaPath))
		fmt.Println("EXEC cat s >> s")
